...
def add_to_cart_service(user, product_id):
    ...
    if product.quantity <= 0:
        raise ValidationError("Sản phẩm hiện đã hết hàng")
    ...
    if not created:
        if order_item.quantity >= product.quantity:
            raise ValidationError("Số lượng trong giỏ đã đạt tồn kho hiện có")
...
def update_quantity_service(user, product_id, quantity):
    ...
    if quantity <= 0:
        ...
        raise ValidationError("Số lượng trong giỏ vượt quá tồn kho hiện có")
